...
bool BME280::begin()
{
	sleep_ms(250);   //Make sure sensor had enough time to turn on. BME280 requires 2ms to start up.
	...
	uint8_t chipID = qwiic.readRegister(I2CAddress, BME280_CHIP_ID_REG); //Should return 0x60 or 0x58
	printf("Chip ID: 0x%x\n", chipID);
	...
	calibration.dig_T1 = ((uint16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_T1_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_T1_LSB_REG)));
	calibration.dig_T2 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_T2_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_T2_LSB_REG)));
	calibration.dig_T3 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_T3_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_T3_LSB_REG)));

	calibration.dig_P1 = ((uint16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P1_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P1_LSB_REG)));
	calibration.dig_P2 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P2_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P2_LSB_REG)));
	calibration.dig_P3 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P3_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P3_LSB_REG)));
	calibration.dig_P4 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P4_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P4_LSB_REG)));
	calibration.dig_P5 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P5_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P5_LSB_REG)));
	calibration.dig_P6 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P6_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P6_LSB_REG)));
	calibration.dig_P7 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P7_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P7_LSB_REG)));
	calibration.dig_P8 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P8_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P8_LSB_REG)));
	calibration.dig_P9 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_P9_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_P9_LSB_REG)));

	calibration.dig_H1 = ((uint8_t)(qwiic.readRegister(I2CAddress, BME280_DIG_H1_REG)));
	calibration.dig_H2 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_H2_MSB_REG) << 8) + qwiic.readRegister(I2CAddress, BME280_DIG_H2_LSB_REG)));
	calibration.dig_H3 = ((uint8_t)(qwiic.readRegister(I2CAddress, BME280_DIG_H3_REG)));
	calibration.dig_H4 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_H4_MSB_REG) << 4) + (qwiic.readRegister(I2CAddress, BME280_DIG_H4_LSB_REG) & 0x0F)));
	calibration.dig_H5 = ((int16_t)((qwiic.readRegister(I2CAddress, BME280_DIG_H5_MSB_REG) << 4) + ((qwiic.readRegister(I2CAddress, BME280_DIG_H4_LSB_REG) >> 4) & 0x0F)));
	calibration.dig_H6 = ((int8_t)qwiic.readRegister(I2CAddress, BME280_DIG_H6_REG));
	...
	return qwiic.readRegister(I2CAddress, BME280_CHIP_ID_REG) == 0x60; //Should return 0x60
}
...
void BME280::setMode(uint8_t mode)
{
	...
	uint8_t controlData = qwiic.readRegister(I2CAddress, BME280_CTRL_MEAS_REG);
	controlData &= ~( (1<<1) | (1<<0) ); //Clear the mode[1:0] bits
	controlData |= mode; //Set
	qwiic.writeRegister(I2CAddress, BME280_CTRL_MEAS_REG, controlData);
}
...
void BME280::setStandbyTime(uint8_t timeSetting)
{
	...
	uint8_t controlData = qwiic.readRegister(I2CAddress, BME280_CONFIG_REG);
	controlData &= ~( (1<<7) | (1<<6) | (1<<5) ); //Clear the 7/6/5 bits
	controlData |= (timeSetting << 5); //Align with bits 7/6/5
	qwiic.writeRegister(I2CAddress, BME280_CONFIG_REG, controlData);
}
...
void BME280::setHumidityOverSample(uint8_t overSampleAmount)
{
	...
	uint8_t controlData = qwiic.readRegister(I2CAddress, BME280_CTRL_HUMIDITY_REG);
	controlData &= ~( (1<<2) | (1<<1) | (1<<0) ); //Clear bits 2/1/0
	controlData |= overSampleAmount << 0; //Align overSampleAmount to bits 2/1/0
	qwiic.writeRegister(I2CAddress, BME280_CTRL_HUMIDITY_REG, controlData);
...
{
	uint8_t stat = qwiic.readRegister(I2CAddress, BME280_STAT_REG);
...
{
	qwiic.writeRegister(I2CAddress, BME280_RST_REG, 0xB6);
...
float BME280::readFloatPressure( void )
{
    ...
    uint8_t buffer[3];
	int rc = qwiic.readRegisterRegion(I2CAddress, BME280_PRESSURE_MSB_REG, buffer, 3);
...
float BME280::readFloatHumidity( void )
{
    ...
    uint8_t buffer[2];
	int rc = qwiic.readRegisterRegion(I2CAddress, BME280_HUMIDITY_MSB_REG, buffer, 2);
...
float BME280::readTempC( void )
{
    ...
    uint8_t buffer[3];
	int rc = qwiic.readRegisterRegion(I2CAddress, BME280_TEMPERATURE_MSB_REG, buffer, 3);
